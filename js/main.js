...
if (!$homeButton)
    throw new Error('$homeButton query failed');
if (!$details)
    throw new Error('$details query failed');
if (!$favorites)
    throw new Error('$favorites query failed');
if (!$favoritesButton)
    throw new Error('$favoritesButton query failed');
...
async function fetchEventData(city, keyword) {
    ...
    try {
        ...
        const serializedEvents = events.map((e) => ({
            ...
            startTime: formatStartTime(e.dates.start.localDate, e.dates.start.localTime),
        }));
...
function writeFavorites(event) {
    const favoritesStorage = localStorage.setItem('favorites-storage', JSON.stringify(event));
...
if (!$header)
    throw new Error('$header query failed');
